...
class Imgur {
    ...
    public function getFavoritesAlbums(string $sUsername = 'mrbellek'): array
    {
        $aFactsDumpAlbums = [];
     ...
     * @return false|int
     */
    public function getAlbumImageCount(string $sUrl)
    {
    ...
    public function getAllAlbumImages(string $sUrl): array
    {
        if (!preg_match('/imgur\.com\/a\/([a-z0-9]+)/i', $sUrl, $aMatch)) {
            if (!preg_match('/imgur\.com\/gallery\/([a-z0-9]+)/i', $sUrl, $aMatch)) {
                return [];
    ...
    public function getFourAlbumImages(string $sUrl, bool $bRandom = true): array
    {
    ...
    private function curlGet(string $sUrl)
    {
        if (empty($this->oCurl)) {
            //init curl
