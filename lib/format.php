...
class Format extends Base
{
     ...
     */
    public function format($oRecord): string
    {
        if (is_array($oRecord)) {
        ...
        switch ($this->config->get('source')) {
            default:
            case 'database':
                return $this->db_format((array)$oRecord);
     ...
     * @param object $oRecord
     */
    public function rss_format($oRecord): string
    {
        $iMaxTweetLength = $this->config->get('max_tweet_lengh', 280);
        $iShortUrlLength = $this->config->get('short_url_length', 23);

        //format message according to format in settings, and return it
        $aTweetVars = $this->config->get('tweet_vars');
        $sTweet = $this->config->get('format');

        //replace all non-truncated fields
        foreach ($aTweetVars as $oTweetVar) {
            if (empty($oTweetVar->truncate)) {
     ...
     */
    public function db_format(array $aRecord): sstring
    {
        $iMaxTweetLength = $this->config->get('max_tweet_lengh', 280);
        $iShortUrlLength = $this->config->get('short_url_length', 23);
    ...
    private function getRssValue($oRecord, $oValue)
    {
        ...
        if (!empty($oValue->regex) && preg_match($oValue->regex, $mReturn, $aMatches)) {
     ...
     * @param object $oValue
     */
    private function getRssSpecialValue($oRecord, $oValue): string
    {
        foreach ($this->config->get('tweet_vars') as $oVar) {
